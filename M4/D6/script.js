...
const ordinaAscendeteDiscendete = document.querySelector("#ordine-ascendente-discendente");
...
function renderUsers(users) {
    ...
    users.forEach((user) => {
        ...
        `
            ...
            <td>
            <a href="infoUtente.html?id=${user.id}">
                    <button style="width: 90px">More Info</button>
                </a>
            </td>
        `;
...
var originalList = [];
var ascendente = true;
ordinaAscendeteDiscendete.addEventListener("click", async () =>{
    const users = await getUsers();
    originalList = users;

    const reversedUsers = originalList.sort((a,b) =>{
        let answer = ascendente ? 1 : -1;
        if(a.name > b.name){
            return answer;
        }else{
            return answer * -1;
